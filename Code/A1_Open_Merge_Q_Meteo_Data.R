...
Q_DC4=read.csv("Input/Q and Meteo/Q_C57_Daily.Time_Series_Data.2024031816461304.csv", skip=11)
...
DC4_Q_Meteo=DC4_Q_Meteo  # Assume DC4 site Q is like DC4
...
ggplot(data=rbind(DC4_Q_Meteo, DC3_Q_Meteo), aes(x=Date, y=Q_m3d, color=Site_id))+
  geom_point()+
...
# Assume q at DC2 is the same as DC4, but fill with DC3 when Na's
DC4_Q_Meteo$q_md_filled= ifelse(is.na(DC4_Q_Meteo$q_md),
                                DC3_Q_Meteo$q_md, 
                                DC4_Q_Meteo$q_md ) # When DC4_qmd is missing, use DC3_qmd
DC2_Q_Meteo$q_md_filled= DC4_Q_Meteo$q_md_filled  # Assume DC2 is like DC4

# Assume q at DC1 is the same at DC3. 
DC3_Q_Meteo$q_md_filled= DC3_Q_Meteo$q_md
DC1_Q_Meteo$q_md_filled= DC3_Q_Meteo$q_md_filled

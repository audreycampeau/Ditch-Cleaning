...
#Code lines to include only DC sites in analysis 
DC_Q = filter(DC_Q, Site_id %in% c( "DC1", "DC2",  "DC3", "DC4"))


# Order factors site pairs 
  DC_Q$Site_id <- factor(DC_Q$Site_id, 
                       levels = c( "DC1", "DC2", "DC3","DC4"))
...
treatments_colors <- c(
  "Pristine" =   "#3CB7CC", # "deepskyblue",     # light blue
  "Clearcut" = "#32A251",#"#CC6677",     # light red
  "Ditch cleaning" = "#FF7F0F" #"#882255" # dark red
)
...
#  "C1" = "#90EE90",  # Light green
  ...
  "FC_2" = "#E69B00", # Light orange
  "FC_4" = "#994F00", #Dark brown
  
  # Clearcut+DitchCleaning pair - disturbed/muddy tones
  "FC_DC1" = "#2E5894", #Dark blue
  "FC_DC3" = "#7EA6E0" #Light blue
)
...
## Figure 1: Timeseries C concentrations and ¹⁴C-content
...
knitr::kable(filter(DC_Q, Site_id %in% c("FC_DC3"))%>%   
             anova_test(DOC_mgL ~ q_md_filled*Treatment), 
             caption = "ANCOVA (DC3) - DOC * q / treatment")

knitr::kable(filter(DC_Q, Site_id %in% c("FC_4"))%>%   
             anova_test(DOC_mgL ~ q_md_filled*Treatment), 
...
## Figure 3: Scatterplot C concentration or ¹⁴C-content by treatment
...
Q_DC3_rank = Wide_q%>%
  group_by(Treatment)%>%
  mutate(rank = rank(-q_md_FC_DC3)) %>%
  mutate(P = 100 * (rank / (length(q_md_FC_DC3) + 1))) %>%
  ungroup()


# Identify dates when 14C-DOC samples were collected
DOC_14C_samples <- DC_Q %>%
  filter(Site_id == "FC_DC3", !is.na(DOC_14C_Modern)) %>%
...
q_exceedance=ggplot(data = Q_DC3_rank) +
  geom_line(aes(y = P, x = q_md_FC_DC3 * 1000, color = Treatment), size = 0.75) +
...
hydrograph= ggplot(DC_Q %>%
         filter(Site_id %in% c("FC_4", "FC_DC3")),
...
ggplot(DC_Q %>% filter(Site_id %in% c("FC_2", "FC_DC3")),

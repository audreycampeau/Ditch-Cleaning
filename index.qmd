...
site_colors <- c(
  # Clearcut pair 
  "FC_1" = "grey10",# "#9e007c", # "#228B22", # Light orange
  "FC_2" = "grey50",#"#451159", # "#90EE90", #Dark brown
  
  # Clearcut+DitchCleaning pair
  "FC_DC1" = "grey10", #"gray60", #"#E69B00", #orange
  "FC_DC2" = "grey50" #"#ccff00" #994F00" #brown
)
...
```{r, warning=F, message=F, width= 12, height=8}
  ...
  ggarrange(
    dotplots_treatment_sites(site_pair = "FC", variable = DOC_mgL)+
     ...
     dotplots_treatment_sites(site_pair = "FC", variable = DOC_14C_Modern)+
      scale_y_log10(limits=c(round(range(DC_Q$DOC_14C_Modern, na.rm=T),0))),
    
    dotplots_treatment_sites(site_pair = "FC", variable = CO2_mgL)+
      scale_y_log10(limits=c(round(range(DC_Q$CO2_mgL, na.rm=T),0))),
    
    dotplots_treatment_sites(site_pair = "FC", variable = CO2_14C_Modern)+
      scale_y_continuous(limits=c(round(range(DC_Q$CO2_14C_Modern, na.rm=T),0))),
    ...
    dotplots_treatment_sites(site_pair = "FC_DC", variable = CO2_14C_Modern)+
      scale_y_log10(limits=c(round(range(DC_Q$CO2_14C_Modern, na.rm=T),0))),
...
ggsave("Output/Figures/Figure3_dotplots.pdf", plot =combined_dotplots, height = 6, width = 12)
...
density_Cscatter_sites_FC    = density_Cscatter_pairs("FC")
...
density_14Cscatter_treatment_FC = density_14Cscatter_pairs(c("FC"))
...
```{r, warning=FALSE, message=FALSE, fig.height=7, fig.width=7, fig.cap="relationship between radiocarbon content of CO2 and DOC and discharge, coloured by treatment"}
...
DOC_Q_scatters_pairs = function(site_pair) {
 ...
 ggplot(DC_Q %>% #filter(!is.na(DOC_14C_Modern))%>%
         filter (Site_pairs==site_pair),
...
CO2_Q_scatters_pairs = function(site_pair) { 
  ggplot(DC_Q %>% #filter(!is.na(CO2_14C_Modern))%>%
          filter (Site_pairs==site_pair),
...
combined_14C_q=ggarrange( plot_DOC_q_FC, plot_DOC_q_FC_DC, 
           plot_CO2_q_FC, plot_CO2_q_FC_DC, labels = "AUTO", align = "hv")
...
extract_lm_results_14C <- function(site_pair, y_var) {
  DC_Q %>%
    filter(Site_pairs == site_pair) %>%
    group_by(Treatment) %>%
    group_map(~ {
...
lm_14C_results <- bind_rows(
  extract_lm_results_14C("FC",     y_var=  "DOC_14C_Modern"),
  extract_lm_results_14C("FC_DC",  y_var=  "DOC_14C_Modern"),
  extract_lm_results_14C("FC",     y_var= "CO2_14C_Modern"),
...
knitr::kable(
  DC_Q %>%
    filter(Site_pairs == "FC", !is.na(DOC_14C_Modern)) %>%
    anova_test(DOC_14C_Modern ~ q_int_mmd * Treatment),
  caption = "ANCOVA results: 14C-DOC ~ discharge × Treatment — FC sites"
)
...
knitr::kable(
  DC_Q %>%
    filter(Site_pairs == "FC", !is.na(CO2_14C_Modern)) %>%
    anova_test(CO2_14C_Modern ~ q_int_mmd * Treatment),
  caption = "ANCOVA results: 14C-CO2 ~ discharge × Treatment — FC sites"
)
...
combined_keeling= ggarrange(plot_14C, 
          plot_d13C, #
          ncol = 1, nrow=2, labels="", heights = c(0.5, 0.5)
          )

combined_keeling
ggsave("Output/Figures/Figure5_keeling.pdf", plot =combined_keeling,
       height= 8, width = 8)

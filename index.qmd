---
title: "Forest manipulation experiment reveals divergent controls on the sources and age of lateral DOC and CO₂ export"
author:
  - name: Audrey Campeau
...
-   LCE and NEE are connected over long timescale, by hydrology [@öquist2014]
...
## Hydrographs and carbon concentrations
...
-   The ^14^C-CO~2~ is doesn't differ significantly across treatments
...
# Hydrological control over  DOC concentration
hydro_resp_DOC=
  ggplot(DC_Q,
       aes(y=DOC_mgL, x=q_md_filled*1000, color=Treatment))+ 
...
hydro_resp_CO2=
  ggplot(DC_Q,
       aes(y=CO2_mgL, x=q_md_filled*1000, color=Treatment))+ 
  geom_point(size=1, show.legend = F)+
  scale_x_log10()+
  scale_y_log10()+
  geom_smooth(method="lm", se=F, aes(color=Treatment), show.legend = T)+
...
Is there a significant difference in the hydrological response of C concentrations between *Treatments*?

```{r, warning=F, echo=FALSE}
#Run ANCOVA test - Difference in hydro. response between treatments
DC_Q  %>%  anova_test(DOC_mgL ~ q_md_filled*Treatment)
DC_Q  %>%  anova_test(DOC_mgL ~ q_md_filled*Site_id)

DC_Q  %>%  anova_test(CO2_mgL ~ q_md_filled*Treatment)
DC_Q  %>%  anova_test(CO2_mgL ~ q_md_filled*Site_id)
```

::: callout-note
#### Interpretation

-   Q **doesn't** have a significant effect on DOC concentration, but treatment and site do (intercept difference). Significant interaction between Q and Site (slope difference)

-   Q has a significant effect on CO2 concentration, but not treatment. Site effect causes a signficant effect on intercept and slope (interaction)
:::
...
ggplot( DC_Q %>% pivot_longer(
  ...
  labs(x="q (m/d)", y=bquote("∆"^14*"C (% modern)"), shape="Watershed ID")+
...
```{r, warning=F, echo=FALSE}
#Run ANCOVA test - Difference in hydro. response between treatments
DC_Q  %>%  anova_test(DOC_14C_Modern ~ q_md_filled*Treatment)
...
```{r, warning=F, echo=FALSE}
#Run ANCOVA test - Difference in hydro. response between treatments
DC_Q%>% anova_test(CO2_14C_Modern ~ q_md_filled*Treatment)
```

```{r, warning=FALSE, message=FALSE}
#Scatter coloured by sites
#ggplot( DC_Q %>% pivot_longer(
#                     cols = c(CO2_14C_Modern, DOC_14C_Modern),
 #                    names_to = "Carbon_specie",
#                     values_to = "C14_value"
#                   ),
#       aes(y=C14_value, x=q_md_filled*1000, fill=Site_id, color=Site_id))+
 # geom_point(size=3)+
#  geom_smooth(method="glm", se=F, aes(color=Site_id), show.legend = F)+
#  scale_color_manual(values=site_colors_6)+ 
...
DC_Q %>% anova_test(DOC_14C_Modern ~ q_md_filled*Site_id)
...
-   there is a significant effect of both **Treatment** **14C-DOC** in the model (intercept differences), but no significant interaction (slope differences). No significant effect of Site_id

-   The intercept shifts from 100%modern in pristine sites, to 110%modern in clearcut+ditchcleaned sites.
:::
...
#Run ANCOVA test - Difference in hydro. response between Sites
DC_Q %>% anova_test(d13C_CO2 ~ CO2_mgL_filled_keeling*Treatment)
...
```{r, warning = F, message = F}
#Run ANCOVA test - Difference in hydro. response between Sites
DC_Q %>% anova_test(CO2_14C_Modern ~ CO2_mgL_filled_keeling*Treatment)
...
-   The keeling plot suggests that CO2 concentration has a significant effect on both d13C-value and 14C-concent, but this relationship doesnt change with treatment or site.

-   The d13C source of CO2 is (-25‰) (no signficant effect of slope or intercept across sites or treatment)

-   The 14C source of CO2 is between 99, 100m and 94. Which are large differences, but don't appear as significant effects in the model.
:::
